...
### Code Integrity
- Linting: Run `flake8` or `ruff` if configured; otherwise ensure no syntax errors by running `./.venv/bin/python -m py_compile $(git ls-files '*.py')`.
- Unit Tests: `./.venv/bin/python -m pytest -q --tb=short` passes with no failures.
- Snapshot Tests: `./.venv/bin/python -m pytest tests/test_prompt_snapshots.py -q --tb=short` pass (base + enriched contexts).
- Type Checking (optional): If `mypy` is configured, run `./.venv/bin/python -m mypy automation/ tests/` and resolve any errors.
- Determinism: Renderer `automation/common/prompt_renderer.py` is pure; enriched/context substitutions produce stable output with identical inputs.

### Deployment
- Environment Variables: Validate `config/env.sample.json` fields used by scripts (`SYSTEM_ENVIRONMENT`, `SYSTEM_OUTPUT_DIRECTORY`, `OUTREACH_*`, `RESUME_*`).
- Build Artifacts: No binary build; generated outputs saved under `output/outreach` and `output/resume` with timestamped filenames.
- Version Tagging: v0.3.5 release notes prepared; canonical current remains `v0.3.0-Phase3C-Normalization`.
- VS Code Tasks: `.vscode/tasks.json` includes env setup, prompt runs (venv + no-sources), tests, metrics, and logs utilities.
- CLI Entrypoints: Combined runner `automation/common/run_prompts.py`; individual scripts for outreach and resume.

### Post-Launch
- Log Monitoring: Events appended to `logs/events.jsonl` (category, timing, output_path). Use task Metrics: Show summary or `automation/common/metrics_cli.py --summary` for counters in `logs/metrics.json`.
- Stakeholder Notification: Share release summary, quickstart commands, and verification results. Provide links to checklist and release notes.
- Known Issues: None blocking. If orchestrator sources are disabled, scripts run with `--no-sources` using sample contexts.
- Support Tasks: Metrics: Reset counters, Metrics: Open JSON, and prompt generation tasks are available in VS Code.

# v0.3.5  Phase3E: Combined CLI + Prompt Rendering
Note: Phase 3E content is FUTURE-only (post-Phase 3C) and not current. This document serves as planning/hardening-oriented release notes aligned to the canonical checklist.
...
This release introduces a unified CLI workflow for generating outreach and resume prompts using enriched job context and userspecific configuration. It finalizes the promptrendering pipeline and adds deterministic output saving with timestamps.

## Highlights

- CLI support for outreach and resume prompt generation
- Deterministic prompt renderer
- Userspecific context file support
- Timestamped output files
- Enrichment toggle respected endtoend
...
- Behaviorlevel logging and metrics
- Snapshot tests for rendered prompts
- Userspecific enrichment overrides

---

## Setup

Use the venvbacked VS Code tasks or terminal commands.
...
- Hyphenated package paths (e.g., automation/job-discovery/...) may cause import errors when running scripts directly. Workaround: run with `PYTHONPATH=$PWD` and prefer dynamic import fallbacks; a hardening fix is targeted for v0.3.6.
...
- If tasks dont appear after editing, run Developer: Reload Window in VS Code
